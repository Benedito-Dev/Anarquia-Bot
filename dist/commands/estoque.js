...
async function ver(interaction) {
    const estoques = db_1.default.prepare("SELECT material, quantidade FROM estoque").all();
    const estoqueMap = {};
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "Cobres", value: `${cobres}`, inline: true }, { name: "Aluminios", value: `${aluminios}`, inline: true }, { name: "Produtos Prontos", value: `${produtos}`, inline: true }, { name: "Pode produzir", value: `${produtosPossiveis} produtos`, inline: true }, { name: "Lotes prontos", value: `${lotesEstoque} lotes (${lotesEstoque * 25} produtos)`, inline: true })
        .setTimestamp();
    await interaction.reply({ embeds: [embed] });
}
async function produzir(interaction) {
    ...
    // Verificar materiais
    const cobres = db_1.default.prepare("SELECT quantidade FROM estoque WHERE material = 'cobres'").get();
    const aluminios = db_1.default.prepare("SELECT quantidade FROM estoque WHERE material = 'aluminios'").get();
    ...
    // Produzir
    const produzirTransaction = db_1.default.transaction(() => {
        db_1.default.prepare("UPDATE estoque SET quantidade = quantidade - ? WHERE material = 'cobres'").run(cobresNecessarios);
        db_1.default.prepare("UPDATE estoque SET quantidade = quantidade - ? WHERE material = 'aluminios'").run(aluminiosNecessarios);
        db_1.default.prepare("UPDATE estoque SET quantidade = quantidade + ? WHERE material = 'produtos'").run(quantidade);
        // Logs
        db_1.default.prepare("INSERT INTO estoque_log (material, quantidade, tipo, descricao, membro_discord_id) VALUES (?, ?, ?, ?, ?)").run("cobres", -cobresNecessarios, "producao", `Producao de ${quantidade} produtos`, discordId);
        db_1.default.prepare("INSERT INTO estoque_log (material, quantidade, tipo, descricao, membro_discord_id) VALUES (?, ?, ?, ?, ?)").run("aluminios", -aluminiosNecessarios, "producao", `Producao de ${quantidade} produtos`, discordId);
        db_1.default.prepare("INSERT INTO estoque_log (material, quantidade, tipo, descricao, membro_discord_id) VALUES (?, ?, ?, ?, ?)").run("produtos", quantidade, "producao", `Producao por ${membro.nome}`, discordId);
        // Log de producao
        db_1.default.prepare("INSERT INTO producao_log (membro_discord_id, quantidade_produtos, cobres_usados, aluminios_usados) VALUES (?, ?, ?, ?)").run(discordId, quantidade, cobresNecessarios, aluminiosNecessarios);
    });
    produzirTransaction();
    const embed = new discord_js_1.EmbedBuilder()
        .setColor(0x2ecc71)
        .setTitle("Producao Concluida!")
        .addFields({ name: "Produtos fabricados", value: `${quantidade}`, inline: true }, { name: "Cobres usados", value: `${cobresNecessarios}`, inline: true }, { name: "Aluminios usados", value: `${aluminiosNecessarios}`, inline: true })
...
async function historico(interaction) {
    const logs = db_1.default
        .prepare("SELECT material, quantidade, tipo, descricao, criado_em FROM estoque_log ORDER BY id DESC LIMIT 10")
        .all();
    if (logs.length === 0) {
